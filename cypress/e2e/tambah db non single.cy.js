context('Tambah Properti Non Single', () => {
  ...
  it('Tambah Data', () => {
    ...
    cy.get('label:contains("Kav") + input').first().type('114A');
    ...
    cy.get('label:contains("No Item") + input').first().type('2801R706942');
    cy.get('label:contains("CL") + input').first().type('134');
    cy.get('label:contains("TRM B") + input').first().type('SOLDER0');
    cy.get('label:contains("Acc bag b1") + input').first().type('Z092-1153-15');
    cy.get('label:contains("Acc bag b2") + input').first().type('-');
    cy.get('label:contains("TBE B") + input').first().type('0097-3358-00');
    cy.get('label:contains("TRM A") + input').first().type('0411-1697-20');
    cy.get('label:contains("Acc bag a1") + input').first().type('0100-6923-17');
    cy.get('label:contains("Acc bag a2") + input').first().type('-');
    cy.get('label:contains("TBE A") + input').first().type('0097-3358-00');
